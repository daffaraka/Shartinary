        ...
        <div className="hidden lg:flex items-center gap-4 shrink-0">
          {isAuthenticated ? (
            <div className="flex items-center gap-4">
               <div className="flex items-center gap-2 px-3 py-1.5 bg-brand-orange/5 rounded-full border border-brand-orange/10">
                  <div className="w-6 h-6 rounded-full bg-brand-brown text-white flex items-center justify-center text-[10px] font-bold uppercase">
                    {user?.name?.substring(0, 2)}
                  </div>
                  <span className="text-[13px] font-bold text-brand-brown">{user?.name}</span>
               </div>
               <button onClick={logout} className="text-[13px] font-bold text-red-500 hover:text-red-700 transition-colors">Keluar</button>
            </div>
          ) : (
            <Link to="/login" className="text-[14px] font-medium text-brand-brown/70 hover:text-brand-brown px-3 py-2">Masuk</Link>
          )}
          ...
          {isAuthenticated ? (
            <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="text-sm font-bold text-brand-brown">{user?.name}</div>
                        <div className="text-xs text-brand-brown/40">{user?.email}</div>
                    </div>
                </div>
                <button onClick={() => { logout(); setIsMobileMenuOpen(false); }} className="text-left text-sm font-bold text-red-500">Keluar dari Akun</button>
            </div>
          ) : (
            <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="text-left text-[16px] font-medium text-brand-brown">Masuk / Daftar</Link>
          )}
