export default function TagList({ data, isLoading, onAdd, onEdit, onDelete }) {
    if (isLoading) {
        return (
    ...
    return (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-brand-brown">Manajemen Tags</h1>
                <button 
                    onClick={onAdd}
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-gray-100 text-gray-400 text-xs uppercase tracking-wider">
                            ...
                            <th className="py-4 px-4">Nama Tag</th>
                            ...
                            data.map((item) => (
                                <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="py-4 px-4">
                                        <span className="bg-brand-brown/5 text-brand-brown px-3 py-1 rounded-lg font-bold text-xs uppercase tracking-wider border border-brand-brown/10">
                                            #{item.name}
                                        </span>
                                    </td>
